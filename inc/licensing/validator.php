...
class Validator {
    ...
    public function check_license( $return_result = false ) {
        ...
        $validation_result = [
            'status'  => 'error',
            'code'    => 0,
            'message' => __( 'License server unreachable. Last known valid state used.', 'pluginrx-agent' )
        ];
        ...
        if ( ! is_wp_error( $response ) ) {
            ...
            if ( $status_code === 200 ) {
                ...
                if ( is_array( $decoded ) && isset( $decoded[ 'status' ] ) ) {
                    ...
                    $validation_result[ 'message' ] = __( 'Invalid response from license server.', 'pluginrx-agent' );
                }
            } else {
                $validation_result[ 'message' ] = __( 'Could not validate. Status code: ', 'pluginrx-agent' ) . $status_code;
            }
        } else {
    ...
    public function invalid_license_notice() {
        ...
        if ( $current_screen->id !== $this->args[ 'settings_screen' ] ) {
            ...
            if ( $code !== 0 && ! $this->has_valid_license() ) {
                ...
                echo wp_kses( sprintf( __( 'Your %1$s license is invalid or expired. Please enter a valid license key in the <a href="%2$s">settings</a>.', 'pluginrx-agent' ),
    ...
    public function get_license_comments() {
        ...
        if ( isset( $_GET[ 'page' ] ) && sanitize_key( wp_unslash( $_GET[ 'page' ] ) ) === $this->args[ 'settings_page' ] ) { // phpcs:ignore
            ...
            if ( $license && isset( $license[ 'status' ] ) ) {
                ...
                $comments = '<span class="license-status ' . sanitize_key( $license[ 'status' ] ) . '" title="' . __( 'Last checked: ', 'pluginrx-agent' ) . '' . $display_time . '">' . wp_kses_post( $license[ 'message' ] ) . '</span>';
            } else {
                /* translators: %s: Plugin author's website link */
                $comments = '<em>' . sprintf( __( 'Make sure to add your website to your account on %s, then paste your License ID here.', 'pluginrx-agent' ),
